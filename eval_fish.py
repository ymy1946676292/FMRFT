...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        img_list = os.listdir(os.path.join(self.args.mot_path, 'Fish_01/images/train', self.seq_num, 'img1'))
        img_list = [os.path.join(self.args.mot_path, 'Fish_01/images/train', self.seq_num, 'img1', _) for _ in img_list
                    if
        ...
        if os.path.exists(os.path.join(self.predict_path, 'pred.txt')):
            os.remove(os.path.join(self.predict_path, 'pred.txt'))
    ...
    def eval_seq(self):
        data_root = os.path.join(self.args.mot_path, 'Fish_01/images/train')
        result_filename = os.path.join(self.predict_path, 'pred.txt')
    ...
    def detect(self, prob_threshold=0.4, area_threshold=100, vis=False):
        ...
        for i in tqdm(range(0, self.img_len)):
            ...
            if vis:
                ...
                gt_boxes = None
                self.visualize_img_with_bbox(cur_vis_img_path, ori_img, dt_instances,
                                             gt_boxes=gt_boxes)

            tracker_outputs = self.tr_tracker.update(dt_instances)
            self.write_results(txt_path=os.path.join(self.predict_path, 'pred.txt'),
                               frame_id=(i + 1),
...
if __name__ == '__main__':
    ...
    # seq_nums = os.listdir(os.path.join(args.mot_path, 'Fish_01/images/train'))
    #
    # seq_nums = sorted(seq_nums)
    # print(seq_nums)
    seq_nums = ['fish01-1','fish01-2',]

    # seq_nums = ['fish02-1', 'fish02-2', 'fish02-4', 'fish02-6','fish02-7']

    # seq_nums = ['fish04-1', 'fish04-3']
    ...
    for seq_num in seq_nums:
        ...
        if not os.path.exists(os.path.join(det.predict_path, 'gt.txt')):
            shutil.copyfile(os.path.join(args.mot_path, 'Fish_01/images/train', seq_num, 'gt/gt.txt'),
                            os.path.join(det.predict_path, 'gt.txt'))

        gt_file = os.path.join(det.predict_path, 'gt.txt')
        pred_file = os.path.join(det.predict_path, 'pred.txt')
        gt_files.append(gt_file)
        pred_files.append(pred_file)

    for gt_file, pred_file in zip(gt_files, pred_files):
        gt = mm.io.loadtxt(gt_file, fmt="mot15-2D", min_confidence=1)  # 读入GT
        ts = mm.io.loadtxt(pred_file, fmt="mot15-2D")  # 读入自己生成的跟踪结果
