...
from models.structures import Instances, Boxes, pairwise_iou, matched_boxlist_iou, gt_unmatched_iou
...
from .decoder import SetCriterion, RTDETR, HybridEncoder, RTDETRTransformer, MiM, HsFusion
...
class ClipMatcher(SetCriterion):
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        # step3. select the untracked gt instances (new_data_path tracks).
        ...
        # step5. update obj_idxes according to the new_data_path matching result.
        track_instances.obj_idxes[new_matched_indices[:, 0]] = gt_instances_i.obj_ids[
            new_matched_indices[:, 1]].long()  # 这个是匹配的图片目标的唯一id
        track_instances.matched_gt_idxes[new_matched_indices[:, 0]] = new_matched_indices[:,
                                                                      1]  # 这个匹配的是分配的1，2，3这些id，代表的也是跟踪目标的id
        ...
        #                                                             Boxes(gt_boxes))  # todo 这个是计算active的框和真实框的iou
        ...
        if len(active_track_boxes) > 0:
            gt_boxes = gt_instances_i.boxes[track_instances.matched_gt_idxes[active_idxes]]
            active_track_boxes = box_ops.box_cxcywh_to_xyxy(active_track_boxes)
            gt_boxes = box_ops.box_cxcywh_to_xyxy(gt_boxes)  # (M,4)
            active_iou = matched_boxlist_iou(Boxes(active_track_boxes),
                                             Boxes(gt_boxes))  # todo 这个是计算active的框和真实框的iou

            track_instances.iou[active_idxes] = active_iou
            ...
            # if self.train:
            if 1 == 2:
                # 首先计算真实框和未匹配的框的iou，并返回匹配的最大的iou值，以及对应的索引值
                unmatched_track_idxes_new = [x for x in unmatched_track_idxes if x not in new_matched_indices[:, 0]]
                unmatched_obj_idxes = track_instances.obj_idxes[unmatched_track_idxes_new]
                for i in unmatched_obj_idxes:
                    assert i < 0, "未匹配的不应该大于0"
                unmatched_track_boxes = track_instances.pred_boxes[unmatched_track_idxes_new]
                unmatched_track_boxes = box_ops.box_cxcywh_to_xyxy(unmatched_track_boxes)
                gt_ious, gt_ious_index = gt_unmatched_iou(Boxes(gt_boxes), Boxes(unmatched_track_boxes))  # (M),(M)

                # 这个是所有的匹配的
                # for i in range(matched_indices.shape[0]):
                #     if track_instances.iou[matched_indices[i, 0]] < gt_ious[matched_indices[i, 1]] and gt_ious[
                #         matched_indices[i, 1]] > 0.7:
                #         # 相当于把真实框和新匹配的给挑出来了  gt unmatched  matched
                #         gt_true_index.append(
                #             torch.stack(
                #                 [matched_indices[i, 1], gt_ious_index[matched_indices[i, 1]], matched_indices[i, 0]],
                #                 dim=0))
                #         # 和真实框的iou计算 unmatched  matched
                #         gt_true_iou.append(
                #             torch.stack([gt_ious[matched_indices[i, 1]], track_instances.iou[matched_indices[i, 0]]],
                #                         dim=0))

                # 这个是之前匹配的
                for i in range(prev_matched_indices.shape[0]):
                    if track_instances.iou[prev_matched_indices[i, 0]] < gt_ious[prev_matched_indices[i, 1]] and \
                            gt_ious[
                                prev_matched_indices[i, 1]] > 0.5:
                        # 相当于把真实框和新匹配的给挑出来了  gt unmatched  matched
                        gt_true_index.append(
                            torch.stack(
                                [prev_matched_indices[i, 1], gt_ious_index[prev_matched_indices[i, 1]],
                                 prev_matched_indices[i, 0]],
                                dim=0))
                        # 和真实框的iou计算 unmatched  matched
                        gt_true_iou.append(
                            torch.stack(
                                [gt_ious[prev_matched_indices[i, 1]], track_instances.iou[prev_matched_indices[i, 0]]],
                                dim=0))

                if len(gt_true_index) != 0:
                    gt_true_index = torch.stack(gt_true_index, dim=0)
                    gt_true_iou = torch.stack(gt_true_iou, dim=0)

                    for i in range(gt_true_index.shape[0]):

                        # if np.random.randint(10) > 6:

                        # 更新未匹配的
                        track_instances.obj_idxes[gt_true_index[i, 1]] = gt_instances_i.obj_ids[
                            gt_true_index[i, 0]].long()  # 这个是匹配的图片目标的唯一id
                        track_instances.matched_gt_idxes[gt_true_index[i, 2]] = gt_true_index[i, 0]
                        track_instances.iou[gt_true_index[i, 1]] = gt_true_iou[i, 0]

                        # track_instances.query_pos[gt_true_index[i, 2]] = track_instances.query_pos[gt_true_index[i, 1]]
                        # track_instances.query_pos_embed[gt_true_index[i, 2]] = track_instances.query_pos_embed[
                        #     gt_true_index[i, 1]]
                        # track_instances.output_embedding[gt_true_index[i, 2]] = track_instances.output_embedding[
                        #     gt_true_index[i, 1]]

                        # 将低iou的去除
                        track_instances.obj_idxes[gt_true_index[i, 2]] = -1
                        track_instances.matched_gt_idxes[gt_true_index[i, 2]] = -1

                        for j in range(len(matched_indices)):
                            if gt_true_index[i, 0] == matched_indices[j, 1]:
                                matched_indices[j, 0] = gt_true_index[i, 1]
...
class RuntimeTrackerBase(object):
    def __init__(self, score_thresh=0.6, filter_score_thresh=0.5, miss_tolerance=5):
        self.score_thresh = score_thresh  # 0.7
        self.filter_score_thresh = filter_score_thresh  # 0.6
        self.miss_tolerance = miss_tolerance  # 5
        self.max_obj_id = 0
    ...
    def forward(self, track_instances: Instances, target_size) -> Instances:
        ...
        Parameters:
            ...
            target_sizes: tensor of dimension [batch_size x 2] containing the size of each images of the batch
                          For evaluation, this must be the original imarge size (before any data augmentation)
...
class MOTR(nn.Module):
    def __init__(self, rtdetr, num_classes, num_queries, num_feature_levels, criterion, track_embed,
        ...
        self.track_embed = track_embed  # track embedding layer
        # self.transformer = transformer  # deformable transformer
        # self.mamba = mamba
        # hidden_dim = mamba.d_model  # 256   todo 后期需要更改

        self.rtdetr = rtdetr

        hidden_dim = 256
        self.num_classes = num_classes  # 1
        # self.class_embed = nn.Linear(hidden_dim, num_classes)  # Linear(256, 1)
        # self.bbox_embed = MLP(hidden_dim, hidden_dim, 4, 3)
        # self.num_feature_levels = num_feature_levels  # 4
        self.use_checkpoint = use_checkpoint  # True
        if not two_stage:
            self.query_embed = nn.Embedding(num_queries, hidden_dim)  # Embedding(300, 512)
        # if num_feature_levels > 1:
        #     num_backbone_outs = len(backbone.strides)  # 3   backbone.strides = [8, 16, 32]
        #     input_proj_list = []
        #     for _ in range(num_backbone_outs):  # backbone.num_channels = [512, 1024, 2048]
        ...
        # num_pred = (transformer.decoder.num_layers + 1) if two_stage else transformer.decoder.num_layers  # 6
        num_pred = 1  # todo 先默认为1，后续再改
        # if with_box_refine:
        ...
        #     # self.transformer.decoder.class_embed = self.class_embedsu
        #     for box_embed in self.bbox_embed:
        #         nn.init.constant_(box_embed.layers[-1].bias.data[2:], 0.0)
        self.post_process = TrackerPostProcess()
        self.track_base = RuntimeTrackerBase()
        self.criterion = criterion
        self.memory_bank = memory_bank  # None
        self.mem_bank_len = 0 if memory_bank is None else memory_bank.max_his_length  # 0

    def _generate_empty_tracks(
            self):  # 生成一个空的轨迹实例（track_instances），这是一个 Instances 对象，其中包含了一系列的轨迹相关属性，所有的属性都被初始化为零或者特定的值。
        track_instances = Instances((1, 1))
        num_queries, dim = self.query_embed.weight.shape  # (300, 256)
        device = self.query_embed.weight.device
        track_instances.ref_pts = self.reference_points(self.query_embed.weight[:, :dim])
        # track_instances.ref_pts = self.mamba.reference_points(self.query_embed.weight[:, :dim // 2])
        track_instances.query_pos = self.query_embed.weight  # (300, 512)
        track_instances.query_pos_embed = torch.zeros((num_queries, dim),
                                                      device=device)  # (300, 512)
        track_instances.output_embedding = torch.zeros((num_queries, dim),
                                                       device=device)  # (300, 256)  dim >> 1 是将 dim 变量进行右移一位操作，相当于将 dim 除以 2
        track_instances.obj_idxes = torch.full((len(track_instances),), -1, dtype=torch.long,
                                               device=device)  # (300, ) 用于存储每个轨迹对应的对象索引。初始化为 -1 表示轨迹开始时没有对应的目标对象。
        track_instances.matched_gt_idxes = torch.full((len(track_instances),), -1, dtype=torch.long,
                                                      device=device)  # (300, ) 用于存储每个轨迹匹配的地面真实目标的索引。初始化为 -1 表示轨迹开始时没有匹配的地面真实目标。
        track_instances.disappear_time = torch.zeros((len(track_instances),), dtype=torch.long,
                                                     device=device)  # (300, ) 用于存储每个轨迹消失的时间。初始化为 0 表示轨迹开始时没有消失。
        track_instances.iou = torch.zeros((len(track_instances),), dtype=torch.float,
                                          device=device)  # (300, ) 用于存储每个轨迹与地面真实目标的 IoU 值。初始化为 0 表示轨迹开始时没有匹配的地面真实目标。
        track_instances.scores = torch.zeros((len(track_instances),), dtype=torch.float,
                                             device=device)  # (300, ) 用于存储每个轨迹的得分。初始化为 0 表示轨迹开始时没有得分。
        track_instances.track_scores = torch.zeros((len(track_instances),), dtype=torch.float, device=device)  #
        track_instances.pred_boxes = torch.zeros((len(track_instances), 4), dtype=torch.float,
                                                 device=device)  # (300, 4) 用于存储每个轨迹的预测框。初始化为 0 表示轨迹开始时没有预测框。
        track_instances.pred_logits = torch.zeros((len(track_instances), self.num_classes), dtype=torch.float,
                                                  device=device)  # (300, 1) 用于存储每个轨迹的预测得分。初始化为 0 表示轨迹开始时没有预测得分。

        mem_bank_len = self.mem_bank_len  # 0 用于存储每个轨迹的历史信息。初始化为 0 表示轨迹开始时没有历史信息。
        track_instances.mem_bank = torch.zeros((len(track_instances), mem_bank_len, dim), dtype=torch.float32,
                                               device=device)  # (300, 0, 256) 用于存储每个轨迹的历史特征。初始化为 0 表示轨迹开始时没有历史特征。
        ...
        track_instances.save_period = torch.zeros((len(track_instances),), dtype=torch.float32,
                                                  device=device)  # (300, ) 用于存储每个轨迹的保存周期。初始化为 0 表示轨迹开始时没有保存周期。
    ...
    def _forward_single_image(self, samples, track_instances: Instances, targets=None):
        ...
        outputs_class, outputs_coord, output_last, reference_points, query_pos_embed, init_query = self.rtdetr(
            samples.tensors,
        ...
        out['ref_pts'] = reference_points
        out['hs'] = output_last
        out['query_pos_embed'] = query_pos_embed
        out['init_query'] = init_query
    ...
    def _post_process_single_image(self, frame_res, track_instances, is_last):
        ...
        track_instances.scores = track_scores  # 对track_instance进行更新
        track_instances.pred_logits = frame_res['pred_logits'][0]
        track_instances.pred_boxes = frame_res['pred_boxes'][0]
        track_instances.output_embedding = frame_res['hs'][0]
        track_instances.query_pos_embed = frame_res['query_pos_embed'][0]
        track_instances.query_pos = frame_res['init_query'][0]
        if self.training:
    ...
    def forward(self, data: dict):
        ...
        track_instances = self._generate_empty_tracks()  # 初始化track_instances
        keys = list(track_instances._fields.keys())
        for frame_index, frame in enumerate(frames):
            frame.requires_grad = False
            is_last = frame_index == len(frames) - 1
            if self.use_checkpoint and frame_index < len(
                    frames) - 2:  # self.use_checkpoint 是一个布尔值，用于决定是否在模型的前向传播过程中使用梯度检查点
                def fn(frame, *args):
                    frame = nested_tensor_from_tensor_list([frame])
                    tmp = Instances((1, 1), **dict(zip(keys, args)))
                    frame_res = self._forward_single_image(frame, tmp, [targets[frame_index]])
                    return (
                        frame_res['pred_logits'],
                        frame_res['pred_boxes'],
                        frame_res['ref_pts'],
                        frame_res['hs'],
                        frame_res['query_pos_embed'],
                        frame_res['init_query'],
                ...
                frame = nested_tensor_from_tensor_list([frame])
                frame_res = self._forward_single_image(frame, track_instances, [targets[frame_index]])
...
def build(args):
    ...
    # 创建mamba模型,使用的为tiny的模型，预训练后期可以加
    kwargs = {"drop_path_rate": 0.0, "drop_rate": 0.0, "img_size": -1,
              "num_classes": -1}  # 由于不是分类，需要将分类头改为nn.Identity()
    # mamba = vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs)
    d_model = 256

    backbone = MiM([2] * 3, [8, 16, 32, 64, 128])

    # fusion = HsFusion([32, 64, 128], hidden_dim=d_model, num_feature_levels=4, with_fpn=True, method_fpn='wbcfpn')

    encoder = HybridEncoder(in_channels=[32, 64, 128])

    decoder = RTDETRTransformer(num_classes=1, num_queries=300, feat_channels=[256, 256, 256],
                                num_denoising=0)  # TODO num_denoising后期优化
    ...
    for i in range(num_frames_per_batch):
        weight_dict.update({"frame_{}_loss_ce".format(i): args.cls_loss_coef,  # 2
                            'frame_{}_loss_bbox'.format(i): args.bbox_loss_coef,  # 5
                            'frame_{}_loss_giou'.format(i): args.giou_loss_coef,  # 2
                            })
    ...
    return model1, criterion, postprocessors
