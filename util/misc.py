...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor], size_divisibility: int = 0):
    ...
    if tensor_list[0].ndim == 3:
        ...
        max_size = _max_by_axis([list(img.shape) for img in tensor_list])  # 因为就一张照片，所以直接取第一张照片的shape
        ...
        tensor = torch.zeros(batch_shape, dtype=dtype, device=device)  # 全为0
        mask = torch.ones((b, h, w), dtype=torch.bool, device=device)  # 因为是一张图片，所以就没有掩码，全为False
